...
// lowercase English letter.  The weight of a word is defined as the sum of the weights of its characters.  For each
// word, take its weight modulo 26 and map the result to a lowercase English letter using reverse alphabetical order 
// (0 -> 'z', 1 -> 'y', ..., 25 -> 'a').  Return a string formed by concatenating the mapped characters for all words
// in order.
//
// Example 1:
...
int main()
{
    ...
    words = { "abcd", "def", "xyz" };
    weights = { 5, 3, 12, 14, 1, 2, 3, 2, 10, 6, 6, 9, 7, 8, 7, 10, 8, 9, 6, 9, 9, 8, 3, 7, 7, 2 };
    std::cout << mapWordWeights(words, weights) << endl;
